 ...
 * user: kcho
 * 
 * action handler for driver accepts request for pickup
 * 
 * GET or POST:
...
do
{
	...
	else if($request['status'] > $REQUEST_STATUS['OPEN'])
	{
		break;
	}
	
	$q = "UPDATE requests SET status=" . $REQUEST_STATUS['ACCEPTED'] . " WHERE requestid=" . $requestid;
	$result = db_query($q);
	if (!$result)
	{
		die('no result from query: ' . $q);
